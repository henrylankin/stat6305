...
Nested model: y(ijk) = mu + tau(i) + beta(j(i)) + error(ijk)
	...
	tau(i) = major effect
	beta(j(i)) = minor effect, nested in tau(i)
*/
...
** Carryover design;
/*
Carryover model: y(ijk) = mu + delta(i) + beta(j(i)) + gamma(k) + tau(l) + { tau*gamma } + error(ijk)
		i = 1,2  j = 1,2,3  k = 1,2,3,4,5
	mu = overall mean
	delta(i) = sequence effect
	beta(j(i)) = subject effect nested in sequence
	gamma(k) = period effect
		- when there is a complete crossover design, the period effect acts as the carryover
	tau(l) = treatment effect
	{ tau*gamma } = carryover effect
		- only can be done for incomplete carryover design
		- when it is complete carryover there won't be enough df to do interaction term
*/

* example with incomplete carryover design;
*** crossover design;
...
data drug_duration;
	do sequence = 1 to 3;
		do subject = 1 to 4;
			do period = 1 to 3;
				do treatment = mod(sequence+period-2, 3) + 1;
					input duration@@;
					output;
				end;
			end;
	...
	cards;
1.5 2.2 3.4 2.0 2.6 3.1 1.6 2.7
3.2 1.1 2.3 2.9 2.5 3.5 1.9 2.8
3.1 1.5 2.7 2.9 2.4 2.4 2.6 2.3
3.3 1.9 2.7 3.1 1.6 2.5 3.6 2.3
2.2 3.0 2.5 2.0
;
run;

/*
data drug_duration; set drug_duration;
...
proc print data=drug_duration;
run;

* run glm proc for crossover;
proc glm data=drug_duration;
	class sequence subject period treatment;
	model duration = sequence subject(sequence) period treatment treatment*period;
		* we can test for the carryover by adding an interaction between treatment and period;
	random subject(sequence) / test; * only subject is random;
run;
quit;

* same thing but using proc mixed;
proc mixed data = drug_duration;
	class sequence subject period treatment;
...
* example with complete crossover design;

* input original data;
data drugfood;
	input subject$ sequence$ AUC_fed AUC_fasted;
	cards;
1	1	809.44	967.82
2	1	428	746.45
3	2	757.71	901.11
...
proc print data = drugfood;
run;

* flatten data and add the treatment column;
data drugfood_2; set drugfood;
	length treatment $ 6;
	treatment = 'fed'; AUC = AUC_fed; output;
	treatment = 'fasted'; AUC = AUC_fasted; output;
run;

proc print data = drugfood_2;
run;

* use if statements to add in the period column 
	[need to add this in after establishing the treatment column, 
	otherwise the if statement will be calling upon an uncreated treatment column];
data drugfood_2; set drugfood_2;
	if sequence = 1 and treatment = 'fed' then period = 1;
	if sequence = 1 and treatment = 'fasted' then period = 2;
	if sequence = 2 and treatment = 'fasted' then period = 1;
	if sequence = 2 and treatment = 'fed' then period = 2;
	keep subject sequence period treatment AUC;
run;
...
proc glm data = drugfood_2; 
	class period sequence;
	model AUC = period sequence sequence*period;
run;
quit;

* run test on crossover model;
proc glm data = drugfood_2;
	class subject sequence period treatment;
	model AUC = sequence subject(sequence) period treatment; *treatment*period;
		* we can test for the carryover by adding an interaction between treatment and period;
